...
export function toBuffer(base64urlString, from = 'base64url') {
    if (typeof base64urlString !== 'string') {
        throw new Error('Expected a Base64URL-encoded string');
    }
    ...
    try {
        ...
        throw new Error('Error decoding Base64URL string: ' + error.message);
    }
}
/**
...
export function isBase64URL(input) {
    console.log('Input for isBAse64URL:', input);
    if (typeof input !== 'string') {
        console.error('Expected a Base64URL string but received:', input);
...
export function trimPadding(input) {
    console.log('Received input for trimming:', input)
    if (typeof input !== 'string') {
